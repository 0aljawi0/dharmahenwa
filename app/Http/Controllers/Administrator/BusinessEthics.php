...
class BusinessEthics extends Controller
{
    ...
    {
        $data = Option::firstWhere('key', 'business-ethics');
        return view('administrator.business_ethics')->with('data', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $option = new Option;
        $option->key = 'business-ethics';
        $option->value = json_encode($value);
        $saved = $option->save();

        // Make log
        Logs::add(Auth::user()->name.' Mengubah business ethics');
