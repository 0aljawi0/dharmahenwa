...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4 animate__animated animate__fadeInDown">
        <h1 class="h3 mb-0 text-gray-800">Edit Stock Price</h1>
    ...
    <div class="row animate__animated animate__fadeInUp">
        ...
        <div class="col-lg-6 mb-4">
            ...
            <form action="{{route('stock-prices.update', ['stock_price' => $stock_price->id])}}" method="POST">
                @csrf
                @method('PUT')
                ...
                @component('administrator.components.input_text')
                    ...
                    @slot('value') {{$stock_price->value}} @endslot
                ...
                <div class="row">
                    <div class="col-lg-4">
                        @component('administrator.components.input_select')
                            ...
                            @for ($i = 1; $i < 32; $i++)
                                <option value="{{$i}}" {{$stock_price->day == $i ? 'selected' : ''}}>{{$i}}</option>
                            @endfor
                        @endcomponent
                    </div>
                    <div class="col-lg-4">
                        @component('administrator.components.input_select')
                            ...
                            @foreach ($month as $key => $item)
                                <option value="{{$key+1}}" {{$stock_price->month == $i ? 'selected' : ''}}>{{$item}}</option>
                            @endforeach
                        @endcomponent
                    </div>
                    <div class="col-lg-4">
                        @component('administrator.components.input_text')
                            ...
                            @slot('type') number @endslot
                            @slot('value') {{$stock_price->year}} @endslot
                            @slot('required') required @endslot
                        @endcomponent
                    </div>
                </div>
