...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4 animate__animated animate__fadeInDown">
        <h1 class="h3 mb-0 text-gray-800">Corporate Policy Manual</h1>
        ...
        <div class="btn-group" role="group">
            ...
            <a href="{{route('policies.create')}}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i class="fas fa-user fa-sm text-white-50"></i> Create New Policy</a>
        </div>
    </div>
            ...
            <table class="table table-striped shadow-sm" id="datatable">
                ...
                <tbody>
                    @foreach ($policies as $key => $item)
                        ...
                        <tr>
                            <td>{{ $key+1 }}</td>
                            <td><a href="{{ asset('storage/'.$item->pdf.'?download=false') }}">File PDF</a></td>
                            <td>{{ $title->en }}</td>
                            <td>{{ date('d F Y H:i:s', strtotime($item->created_at)) }}</td>
                            <td>
                                <div class="btn-group" role="group">
                                    <a href="{{route('policies.edit', ['policy' => $item->id])}}" class="btn btn-info btn-sm"> <i class="fas fa-pencil-alt fa-sm fa-fw"></i> <span>Edit</span></a>
                                ...
                                <form id="destroy_{{$item->id}}" action="{{route('policies.destroy', ['policy' => $item->id])}}" method="POST"> @csrf @method('DELETE') </form>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
